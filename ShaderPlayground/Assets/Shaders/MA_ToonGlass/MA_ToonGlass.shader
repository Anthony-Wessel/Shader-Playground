...
#pragma surface surf ToonRamp keepalpha
		...
		#pragma lighting ToonRamp exclude_path:prepass
			...
			void surf(Input IN, inout SurfaceOutput o) {
				...
				half3 rampS = tex2D(_RampS, float2(lightDot, lightDot)).rgb; // specular ramp
				...
				half3 rampVS = tex2D(_RampS, float2(viewSpec, viewSpec)).rgb; // view specular ramp
				float viewSpecLine = (smoothstep(_SpecSize2, _SpecSize2 + _Offset, viewSpec)) * 10;

				// light dir specular
				float specular = (smoothstep(_SpecSize, _SpecSize + _Offset2, rampS.r)) * lightDot * 10;
